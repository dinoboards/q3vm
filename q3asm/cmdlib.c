...
int Q_stricmp(const char *s1, const char *s2) { return Q_strncasecmp(s1, s2, 99999); }
...
void CRC_ProcessByte(unsigned short *crcvalue, byte data) { *crcvalue = (*crcvalue << 8) ^ crctable[(*crcvalue >> 8) ^ data]; }
