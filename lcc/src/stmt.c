...
static void swstmt(int loop, int lab, int lev) {
  ...
  if (generic(e->op) == INDIR && isaddrop(e->kids[0]->op) && e->kids[0]->u.sym->type == e->type &&
...
void swcode(Swtch swp, int b[], int lb, int ub) {
  ...
  if (u - l + 1 <= 3) {
    ...
    walk(tree(JUMP, voidtype, rvalue((*optree['+'])(ADD, pointer(idtree(table)), e)), NULL), 0, 0);
    code(Switch);
...
void retcode(Tree p) {
  ...
  if (retv) {
    if (iscallb(p))
      p = tree(RIGHT, p->type, tree(CALL + B, p->type, p->kids[0]->kids[0], idtree(retv)), rvalue(idtree(retv)));
    else
...
void definelab(int lab) {
  ...
  for (cp = codelist->prev; cp->kind <= Label;)
    cp = cp->prev;
  while (cp->kind == Jump && cp->u.forest->kids[0] && specific(cp->u.forest->kids[0]->op) == ADDRG + P &&
...
static int foldcond(Tree e1, Tree e2) {
  ...
  if (e1 == 0 || e2 == 0)
    return 0;
  if (generic(e1->op) == ASGN && isaddrop(e1->kids[0]->op) && generic(e1->kids[1]->op) == CNST) {
    v  = e1->kids[0]->u.sym;
    e1 = e1->kids[1];
  } else
    return 0;
  if ((op == LE || op == LT || op == EQ || op == NE || op == GT || op == GE) && generic(e2->kids[0]->op) == INDIR &&
