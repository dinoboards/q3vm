...
static void acaller(char *caller, char *file, int x, int y, int count, struct func *callee) {
  ...
  if (!q) {
    ...
    q->y     = y;
    q->count = 0;
    for (r = &callee->callers;
         *r && (strcmp(q->name, (*r)->name) > 0 || strcmp(q->file, (*r)->file) > 0 || q->y > (*r)->y || q->y > (*r)->y);
         r = &(*r)->link)
      ;
    q->link = *r;
    *r      = q;
...
int process(char *file) {
  ...
  if ((fp = fopen(file, "r")) != NULL) {
    ...
    if (more < 0)
      return more;
    for (p = filelist; p; p = p->link)
      qsort(p->counts, p->count, sizeof *p->counts, (int (*)(const void *, const void *))compare);
