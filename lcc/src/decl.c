...
static Type specifier(int *sclass) {
  ...
  if ((size == SHORT && type != INT) || (size == LONG + LONG && type != INT) || (size == LONG && type != INT && type != DOUBLE) ||
...
static void decl(Symbol (*dcl)(int, char *, Type, Coordinate *)) {
  ...
  } else if (ty == NULL || !(isenum(ty) || (isstruct(ty) && (*unqual(ty)->u.sym->name < '1' || *unqual(ty)->u.sym->name > '9'))))
...
static void checkref(Symbol p, void *cl) {
  ...
  if (p->sclass == AUTO && ((p->scope == PARAM && regcount == 0) || p->scope >= LOCAL) && !p->addressed && isscalar(p->type) &&
      p->ref >= 3.0)
    p->sclass = REGISTER;
