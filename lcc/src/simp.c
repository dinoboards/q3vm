...
#define xfoldcnst(TYPE, VAR, OP, FUNC)                                                                                             \
  if (l->op == CNST + TYPE && r->op == CNST + TYPE &&                                                                              \
      FUNC(l->u.v.VAR, r->u.v.VAR, ty->u.sym->u.limits.min.VAR, ty->u.sym->u.limits.max.VAR, needconst))                           \
  return cnsttree(ty, l->u.v.VAR OP r->u.v.VAR)
#define xcvtcnst(FTYPE, SRC, DST, VAR, EXPR)                                                                                       \
  if (l->op == CNST + FTYPE)                                                                                                       \
    do {                                                                                                                           \
      if (!explicitCast && ((SRC) < DST->u.sym->u.limits.min.VAR || (SRC) > DST->u.sym->u.limits.max.VAR))                         \
        warning("overflow in converting constant expression from "                                                                 \
                "`%t' to `%t'\n",                                                                                                  \
                l->type, DST);                                                                                                     \
      if (needconst || !((SRC) < DST->u.sym->u.limits.min.VAR || (SRC) > DST->u.sym->u.limits.max.VAR))                            \
...
#define zerofield(OP, TYPE, VAR)                                                                                                   \
  ...
  return eqtree(                                                                                                                   \
      OP, bittree(BAND, l->kids[0], cnsttree(unsignedtype, (unsigned long)fieldmask(l->u.field) << fieldright(l->u.field))), r)
...
static int addi(long x, long y, long min, long max, int needconst) {
  int cond = x == 0 || y == 0 || (x < 0 && y < 0 && x >= min - y) || (x < 0 && y > 0) || (x > 0 && y < 0) ||
             (x > 0 && y > 0 && x <= max - y);
  if (!cond && needconst) {
...
static int muli(long x, long y, long min, long max, int needconst) {
  int cond = (x > -1 && x <= 1) || (y > -1 && y <= 1) || (x < 0 && y < 0 && -x <= max / -y) || (x < 0 && y > 0 && x >= min / y) ||
...
Tree simplify(int op, Type ty, Tree l, Tree r) {
  ...
  case ADD + P:
    ...
    if (l->op == ADD + P && isaddrop(l->kids[1]->op) &&
        ((r->op == CNST + I && r->u.v.i <= longtype->u.sym->u.limits.max.i && r->u.v.i >= longtype->u.sym->u.limits.min.i) ||
         (r->op == CNST + U && r->u.v.u <= longtype->u.sym->u.limits.max.i)))
      return simplify(ADD + P, ty, l->kids[0], addrtree(l->kids[1], cast(r, longtype)->u.v.i, ty));
    ...
    if (l->op == ADD + I && generic(l->kids[1]->op) == CNST && r->op == ADD + P && generic(r->kids[1]->op) == CNST)
      return simplify(ADD + P, ty, l->kids[0],
                      simplify(ADD + P, ty, r->kids[0], simplify(ADD, r->kids[1]->type, l->kids[1], r->kids[1])));
  ...
  case DIV + I:
    ...
    if ((r->op == CNST + I && r->u.v.i == 0) ||
        (l->op == CNST + I && l->u.v.i == ty->u.sym->u.limits.min.i && r->op == CNST + I && r->u.v.i == -1))
      break;
    xfoldcnst(I, i, /, divi);
    break;
  case DIV + U:
  ...
  case SUB + P:
    ...
    if (r->op == CNST + I || r->op == CNST + U)
      return simplify(ADD, ty, l, cnsttree(inttype, r->op == CNST + I ? -r->u.v.i : -(long)r->u.v.u));
