...
#define iscall(op)                                                                                                                 \
  (generic(op) == CALL ||                                                                                                          \
   (IR->mulops_calls && (generic(op) == DIV || generic(op) == MOD || generic(op) == MUL) && (optype(op) == U || optype(op) == I)))
static Node forest;
static struct dag {
  struct node node;
  struct dag *hlink;
      ...
      if (fieldsize(f) < 8 * f->type->size) {
        ...
        if ((q->op == CNST + I && q->u.v.i == 0) || (q->op == CNST + U && q->u.v.u == 0))
          ...
          q = bittree(BOR, bittree(BAND, rvalue(lvalue(x)), cnsttree(unsignedtype, (unsigned long)~mask)),
                      bittree(BAND, shtree(LSH, cast(q, unsignedtype), cnsttree(unsignedtype, (unsigned long)fieldright(f))),
